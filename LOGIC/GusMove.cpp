...
void GusMove::Init()
{
	...
	collider->RegisterEnterFunction(&GusMove::Enter, this);
	collider->RegisterStayFunction(&GusMove::Stay, this);
	collider->RegisterExitFunction(&GusMove::Exit, this);
...
void GusMove::Enter(Collider* _This, Collider* _Other)
{
	if (_Other->GetOrder() == CG_BALL)
	{
	 ...
	 if (_Other->GetOrder() == CG_FOOD && PetList::existGus == true)
	{
		 Sound::Play(L"SLURP.wav");

		FoodMove::PopFoodActor((BoundingSphere*)_Other);
...
void GusMove::Stay(Collider* _This, Collider* _Other)
{
	 if (_Other->GetOrder() == CG_FOOD && PetList::existGus == true)
	{
		FoodMove::PopFoodActor((BoundingSphere*)_Other);
...
void GusMove::Exit(Collider* _This, Collider* _Other)
{
	int a = 0;
}
